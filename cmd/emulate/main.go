...
func runStart(ctx context.Context, args []string, stdout io.Writer, stderr io.Writer) int {
	...
	if *seedValue != "" {
		...
		if raw, ok := loaded.Data["google"]; ok {
			var cfg google.SeedConfig
			if err := json.Unmarshal(raw, &cfg); err != nil {
				fmt.Fprintf(stderr, "Failed to parse google seed config: %v\n", err)
				return 1
			}
			googleSeed = &cfg
		}
		...
		if raw, ok := loaded.Data["stripe"]; ok {
			var cfg stripe.SeedConfig
			if err := json.Unmarshal(raw, &cfg); err != nil {
				fmt.Fprintf(stderr, "Failed to parse stripe seed config: %v\n", err)
				return 1
			}
			stripeSeed = &cfg
		}
