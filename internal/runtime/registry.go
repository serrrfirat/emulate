...
var Services = []Service{
	...
	{Name: "aws", Label: "AWS cloud service emulator", Endpoints: "S3 (buckets, objects), SQS (queues, messages), SNS (topics, subscriptions), EventBridge (buses, rules, targets), DynamoDB (tables, items), CloudWatch Logs (groups, streams, events), Secrets Manager (secrets, versions), IAM (users, roles, access keys), STS (assume role, caller identity)", InitConfig: map[string]any{"region": "us-east-1", "s3": map[string]any{"buckets": []map[string]any{{"name": "my-app-bucket"}, {"name": "my-app-uploads"}}}, "sqs": map[string]any{"queues": []map[string]any{{"name": "my-app-events"}, {"name": "my-app-dlq"}}}, "secretsmanager": map[string]any{"secrets": []map[string]any{{"name": "my-app/database-url", "secret_string": "postgres://localhost:5432/app"}}}, "iam": map[string]any{"users": []map[string]any{{"user_name": "developer", "create_access_key": true}}, "roles": []map[string]any{{"role_name": "lambda-execution-role", "description": "Role for Lambda function execution"}}}}},
